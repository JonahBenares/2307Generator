				...
				<page size="Legal" class="page-break">
					<div class="p-2 !relative text-center bg-gren-900">
						...
						<div class="first-quarter">
							<span v-for="first in d.firstmonth"> {{ d.tax_type === "Vat" ? parseFloat(first.amount / 1.12).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') :  parseFloat(first.amount,2).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }} <br></span>
						</div>
						<div class="first-total" v-if="d.firstmonth !=0">{{ parseFloat(d.subtotal_first).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div>
						<div class="first-total" v-else>-</div>

						<div class="second-quarter">
							<span v-for="second in d.secondmonth">{{ d.tax_type === "Vat" ? parseFloat(second.amount / 1.12).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') : parseFloat(second.amount,2).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}<br></span>
						</div>
						<div class="second-total"  v-if="d.secondmonth !=0">{{ parseFloat(d.subtotal_second).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div>
						<div class="second-total" v-else>-</div>

						<div class="third-quarter">
							<span v-for="third in d.thirdmonth">{{ d.tax_type === "Vat" ? parseFloat(third.amount / 1.12).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') : parseFloat(third.amount,2).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}<br></span>
						</div>
						<div class="third-total" v-if="d.thirdmonth !=0">{{ parseFloat(d.subtotal_third).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div>
						...
						<div class="grand-total" v-if="d.grandtotal !=0">{{ parseFloat(d.grandtotal).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div>
						...
						<div class="finaltax">{{ parseFloat(d.totaltax).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div> <!-- para ni sa printing nga class naka-->
						<div class="final-tax ">{{ parseFloat(d.totaltax).toFixed(2).replace(/(\d)(?=(\d{3})+\.)/g, '$1,') }}</div><!-- para ni ya sa view nga class-->
